    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div>
                ...
                <input type="text" id="entry-name" value={name} onChange={e => setName(e.target.value)} required className="mt-1 block w-full bg-background dark:bg-surface-accent border border-surface-accent dark:border-gray-600 rounded-md shadow-sm py-2 px-3 text-text-primary focus:outline-none focus:ring-primary focus:border-primary sm:text-sm" />
            </div>
            <div>
                <label htmlFor="entry-value" className="block text-sm font-medium text-text-secondary">Value</label>
                <input type="number" id="entry-value" value={value} onChange={e => setValue(+e.target.value)} required min="0" step="any" className="mt-1 block w-full bg-background dark:bg-surface-accent border border-surface-accent dark:border-gray-600 rounded-md shadow-sm py-2 px-3 text-text-primary focus:outline-none focus:ring-primary focus:border-primary sm:text-sm" />
            </div>
                ...
                {historicalData.length > 0 ? (
                    <ResponsiveContainer width="100%" height="90%">
                        <LineChart data={historicalData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                            ...
                            <Tooltip contentStyle={{ backgroundColor: 'var(--chart-tooltip-bg)', border: `1px solid var(--chart-tooltip-border)`, color: 'var(--color-text-primary)'}} />
                            <Legend />
