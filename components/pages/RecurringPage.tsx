        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-surface-accent">
            ...
            <tbody className="bg-surface divide-y divide-surface-accent">
              {recurringTransactions.length > 0 ? recurringTransactions.map((rt, index) => {
                const isExpense = rt.type === TransactionType.EXPENSE;
                return (
                  <tr key={rt.id} className={`${index % 2 === 0 ? 'bg-surface' : 'bg-surface-accent/20'} hover:bg-surface-accent/40`}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-text-primary">{rt.description}</td>
